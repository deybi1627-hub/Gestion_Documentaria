<?php

namespace App\Policies;

use Illuminate\Auth\Access\Response;
use App\Models\Tramite;
use App\Models\User;

class TramitePolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return in_array($user->role, ['admin', 'secre']);
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Tramite $tramite): bool
    {
        // El usuario puede ver su propio trámite
        if ($tramite->user_id === $user->id) {
            return true;
        }

        // Admin y secretario pueden ver todos los trámites
        return in_array($user->role, ['admin', 'secre']);
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        return true; // Todos los usuarios autenticados pueden crear trámites
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Tramite $tramite): bool
    {
        // Solo admin y secretario pueden actualizar trámites
        return in_array($user->role, ['admin', 'secre']);
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Tramite $tramite): bool
    {
        // Solo admin puede eliminar trámites
        return $user->role === 'admin';
    }

    /**
     * Determine whether the user can manage tramite states.
     */
    public function manage(User $user): bool
    {
        return in_array($user->role, ['admin', 'secre']);
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Tramite $tramite): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Tramite $tramite): bool
    {
        return false;
    }
}
